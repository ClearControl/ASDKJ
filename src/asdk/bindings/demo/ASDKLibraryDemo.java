...
public class ASDKLibraryDemo
{
  ...
  public void demo()
  {
    ...
    Pointer<Byte> lPointerToSerialNumber =
                                         Pointer.pointerToCString(lSerialName);
    System.out.println("ASDKLibrary.asdkInit(...");
    Pointer<DM> lDevicePointer =
                               ASDKLibrary.asdkInit(lPointerToSerialNumber);
    ASDKLibrary.asdkPrintLastError();
    assertNotNull(lDevicePointer);

    Pointer<Double> lPointerToNumberOfActuators =
    ...
    Pointer<Double> lData =
                          Pointer.allocateDoubles(lNumberOfActuators);
    for (int i = 0; i < lNumberOfActuators; i++)
      lData.set(i, 0.1 * (2 * random() - 1));

    System.out.println("ASDKLibrary.asdkSend(...");
    IntValuedEnum<COMPL_STAT> lAsdkSend =
                                        ASDKLibrary.asdkSend(lDevicePointer,
                                                             lData);
