...
public class AlpaoDeformableMirror implements AutoCloseable
{
  ...
  public boolean open()
  {
    if (mDevicePointer != null)
      return false;
    final Pointer<Byte> lPointerToSerialNumber =
                                               Pointer.pointerToCString(mAlpaoDeviceSerialName);
    if (isDebugPrintout())
  ...
  {
    final Pointer<Double> lPointerToNumberOfActuators =
    ...
    if (isDebugPrintout())
      printLastError();
    final int lNumberOfActuators =
                                 (int) lPointerToNumberOfActuators.getDouble();
  ...
  {
    if (isDebugPrintout())
      System.out.println("ASDKLibrary.asdkSet(...TriggerIn..."
                         + pTriggerMode);
  ...
  public boolean setLogPrintLevel(int level)
  {
    if (level < 0 || level > 4)
    {
      if (isDebugPrintout())
      {
        System.out.println("LogPrintLevel " + level
    ...
    if (isDebugPrintout())
      System.out.println("ASDKLibrary.asdkSet(...LogPrintLevel..."
                         + level);
  ...
  {
    final int lMatrixHeightWidth =
                                 AlpaoDeformableMirrorsSpecifications.getFullMatrixHeightWidth(getNumberOfActuators());
    final Pointer<Double> lPointerToDoubles =
                                            Pointer.pointerToDoubles(new double[lMatrixHeightWidth
                                                                                * lMatrixHeightWidth]);
  ...
  {
    final Pointer<Double> lPointerToDoubles =
                                            Pointer.pointerToDoubles(pFullMatrixMirrorShapeVector);
    final boolean lReturnValue =
                               sendFullMatrixMirrorShapeVector(lPointerToDoubles);
  ...
  private Pointer<Double> removeNonExistantCornerActuators(Pointer<Double> pSquareMirrorShapeVectorDoubleBuffer)
  {
    ...
    if (mRawMirrorShapeVector == null
        || mRawMirrorShapeVector.getValidElements() != getNumberOfActuators())
      mRawMirrorShapeVector =
                            Pointer.allocateDoubles(getNumberOfActuators());
  ...
  {
    final Pointer<Double> lPointerToDoubleArray =
                                                Pointer.pointerToDoubles(pMirrorShape);
    final boolean lReturnValue =
                               sendRawMirrorShapeVector(lPointerToDoubleArray);
                                                       ...
                                                       final int pNumberOfRepeats)
  {
    ...
    final Pointer<Double> lPointerToDoubleArray =
                                                Pointer.pointerToDoubles(pMirrorShapeSequence);
    final boolean lReturnValue =
                               sendMirrorShapeSequenceAsynchronously(lPointerToDoubleArray,
                                                                     pNumberOfPatterns,
                                                                     pNumberOfRepeats);
    lPointerToDoubleArray.release();
    return lReturnValue;
